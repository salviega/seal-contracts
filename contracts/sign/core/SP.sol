...
contract SP is ISP, UUPSUpgradeable, OwnableUpgradeable {
	...
	function initialize(
		uint64 schemaCounter_,
		uint64 attestationCounter_
	) public initializer {
	...
	) external override returns (uint64 schemaId) {
		bool delegateMode = delegateSignature.length != 0;
		if (delegateMode) {
			__checkDelegationSignature(
				schema.registrant,
				getDelegatedRegisterHash(schema),
				delegateSignature
			);
		} else {
	...
	function registerBatch(
		...
		bytes calldata delegateSignature
	) external override returns (uint64[] memory schemaIds) {
		bool delegateMode = delegateSignature.length != 0;
		address registrant = schemas[0].registrant;
		if (delegateMode) {
			// solhint-disable-next-line max-line-length
			__checkDelegationSignature(
				schemas[0].registrant,
				getDelegatedRegisterBatchHash(schemas),
				delegateSignature
			);
		} else {
	...
	function attest(
		...
		if (delegateMode) {
			__checkDelegationSignature(
				...
				getDelegatedAttestHash(attestation),
				delegateSignature
			);
		}
		(uint64 schemaId, uint64 attestationId) = _attest(
			attestation,
			indexingKey,
			delegateMode
		);
	...
	) external override returns (uint64[] memory attestationIds) {
		bool delegateMode = delegateSignature.length != 0;
		address attester = attestations[0].attester;
		if (delegateMode) {
			__checkDelegationSignature(
				attester,
				getDelegatedAttestBatchHash(attestations),
				delegateSignature
			);
		}
	...
	function attestBatch(
		...
		bytes memory delegateSignature,
		bytes memory extraData
	) external payable override returns (uint64[] memory attestationIds) {
		bool delegateMode = delegateSignature.length != 0;
		address attester = attestations[0].attester;
		if (delegateMode) {
			__checkDelegationSignature(
	...
	function attestBatch(
		...
		for (uint256 i = 0; i < attestations.length; i++) {
			if (
				delegateMode && attestations[i].attester != attestations[0].attester
			) {
				revert AttestationWrongAttester();
			}
			(uint64 schemaId, uint64 attestationId) = _attest(
	...
	function attestOffchain(
		...
		if (delegateSignature.length != 0) {
			__checkDelegationSignature(
				delegateAttester,
				getDelegatedOffchainAttestHash(offchainAttestationId),
				delegateSignature
			);
	...
	function attestOffchainBatch(
		...
		if (delegateSignature.length != 0) {
			__checkDelegationSignature(
				delegateAttester,
				getDelegatedOffchainAttestBatchHash(attestationIds),
				delegateSignature
			);
			attester = delegateAttester;
		}
	...
	function revoke(
		...
		address storageAttester = _getSPStorage()
			.attestationRegistry[attestationId]
			.attester;
		bool delegateMode = delegateSignature.length != 0;
		if (delegateMode) {
			__checkDelegationSignature(
	...
	function revokeBatch(
		...
		if (delegateMode) {
			address storageAttester = _getSPStorage()
				.attestationRegistry[attestationIds[0]]
				.attester;
			__checkDelegationSignature(
				storageAttester,
				getDelegatedRevokeBatchHash(attestationIds, reasons),
				delegateSignature
			);
		...
		for (uint256 i = 0; i < attestationIds.length; i++) {
			address storageAttester = _getSPStorage()
				.attestationRegistry[attestationIds[i]]
				.attester;
	...
	function revoke(
		...
		ISPHook hook = __getResolverFromAttestationId(attestationId);
		if (address(hook) != address(0)) {
			hook.didReceiveRevocation{value: resolverFeesETH}(
				storageAttester,
				schemaId,
				attestationId,
				extraData
	...
	function revokeOffchain(
		...
		if (delegateMode) {
			...
			__checkDelegationSignature(
				storageAttester,
				getDelegatedOffchainRevokeHash(offchainAttestationId, reason),
				delegateSignature
			);
		}
	...
	function revokeOffchainBatch(
		...
		if (delegateMode) {
			address storageAttester = _getSPStorage()
				.offchainAttestationRegistry[offchainAttestationIds[0]]
				.attester;
			__checkDelegationSignature(
				storageAttester,
				getDelegatedOffchainRevokeBatchHash(offchainAttestationIds, reasons),
				delegateSignature
			);
			currentAttester = storageAttester;
		}
		for (uint256 i = 0; i < offchainAttestationIds.length; i++) {
			address storageAttester = _getSPStorage()
				.offchainAttestationRegistry[offchainAttestationIds[i]]
				.attester;
			if (delegateMode && storageAttester != currentAttester) {
				revert AttestationWrongAttester();
			}
	...
	function getOffchainAttestation(
		string calldata offchainAttestationId
	) external view returns (OffchainAttestation memory) {
	...
	function getDelegatedRevokeBatchHash(
		...
		return
			keccak256(abi.encode(REVOKE_BATCH_ACTION_NAME, attestationIds, reasons));
	}
	...
	function getDelegatedOffchainRevokeHash(
		...
		return
			keccak256(
				abi.encode(REVOKE_OFFCHAIN_ACTION_NAME, offchainAttestationId, reason)
			);
	}
	...
	function getDelegatedOffchainRevokeBatchHash(
		...
		return
			keccak256(
				abi.encode(
					REVOKE_OFFCHAIN_BATCH_ACTION_NAME,
	...
	function _callGlobalHook() internal {
		...
		if (address($.globalHook) != address(0))
			$.globalHook.callHook(_msgData(), _msgSender());
	}
	...
	function _attest(
		...
		string memory indexingKey,
		bool delegateMode
	) internal returns (uint64 schemaId, uint64 attestationId) {
		SPStorage storage $ = _getSPStorage();
		if ($.paused) revert Paused();
		...
		if (
			attestation.linkedAttestationId != 0 &&
			$.attestationRegistry[attestation.linkedAttestationId].attester !=
			_msgSender()
		) {
			revert AttestationWrongAttester();
		}
	...
	function _attestOffchain(
		string calldata offchainAttestationId,
		address attester
	) internal {
		SPStorage storage $ = _getSPStorage();
		if ($.paused) revert Paused();
		OffchainAttestation storage attestation = $.offchainAttestationRegistry[
	...
	function __offchainAttestationExists(
